...
def get_image(robotID,width=200,height=200):
	...
	return I,depthImg_buffer,segImg


def get_default_camera_image(view_matrix,width=400,height=400):
	projection_matrix = p.computeProjectionMatrixFOV(fov,aspect,near,far)
	_,_,rgbImg,depthImg_buffer,segImg=p.getCameraImage(width,height,view_matrix,projection_matrix,renderer=p.ER_TINY_RENDERER)#,lightAmbientCoeff=random.random())
	# _,_,rgbImg,depthImg_buffer,segImg=p.getCameraImage(width,height)    
	# _,_,rgbImg,depthImg_buffer,segImg=p.getCameraImage(width,height,view_matrix,projection_matrix)    
	A = np.reshape(rgbImg, (width,height, 4))[:, :, :3]  
	depthImg_buf = np.reshape(depthImg_buffer, [width, height])
	depthImg = far * near / (far - (far - near) * depthImg_buf)
	# Image.fromarray(A)
